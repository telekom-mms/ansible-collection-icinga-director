...
for module in ../plugins/modules/*.py; do
    ...
    if [[ $module == *info* ]]; then
      ...
      for item in $(seq 0 ${length}); do
        ...
        yq w -i "../tests/integration/targets/icinga/roles/icinga/tasks/wrong_query_${module_name}.yml" "(name==*).\"${fqcn_name}\".query" "noobjecttofind"
      ...
      echo  "---" | \
        tee "../examples/${module_name}.yml" \
            "../tests/integration/targets/icinga/roles/icinga/tasks/${module_name}.yml" \
            "../tests/integration/targets/icinga/roles/icinga/tasks/absent_${module_name}.yml" \
            "../tests/integration/targets/icinga/roles/icinga/tasks/wrong_pass_${module_name}.yml" \
      ...
      for item in $(seq 0 ${length}); do
        ...
        # yq does this by appending the contents of "assert_fail.yml" to the wrong_pass/host tests
        yq m -a -i "../tests/integration/targets/icinga/roles/icinga/tasks/wrong_pass_${module_name}.yml" assert_fail.yml
        yq m -a -i "../tests/integration/targets/icinga/roles/icinga/tasks/wrong_host_${module_name}.yml" assert_fail.yml

        # this replaces "state: present" with "state: absent" in the absent_*.yml tasks
        yq w -i "../tests/integration/targets/icinga/roles/icinga/tasks/absent_${module_name}.yml" "(name==*).\"${fqcn_name}\".state" "absent"
        ...
        yq d -i "../tests/integration/targets/icinga/roles/icinga/tasks/absent_${module_name}.yml" "(name==*).\"${fqcn_name}\".imports"
        yq d -i "../tests/integration/targets/icinga/roles/icinga/tasks/absent_${module_name}.yml" "(name==*).\"${fqcn_name}\".command"
        ...
        yq w -i "../tests/integration/targets/icinga/roles/icinga/tasks/wrong_host_${module_name}.yml" "(name==*).\"${fqcn_name}\".url" "http://nonexistant"
