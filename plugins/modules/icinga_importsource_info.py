...
DOCUMENTATION = """
---
module: icinga_importsource_info
short_description: Query import sources in Icinga2 Director
description:
   - Get a list of import source objects from Icinga2 Director through the director API.
...
options:
  query:
    description:
      ...
      - The text is matched on source_name.
      - Only import sources containing this text will be returned in the resultset.
...
EXAMPLES = """
- name: Query all import sources in icinga
  telekom_mms.icinga_director.icinga_importsource_info:
    url: "{{ icinga_url }}"
    url_username: "{{ icinga_user }}"
    url_password: "{{ icinga_pass }}"

- name: Query a specific import source in icinga
  telekom_mms.icinga_director.icinga_importsource_info:
    url: "{{ icinga_url }}"
    url_username: "{{ icinga_user }}"
    url_password: "{{ icinga_pass }}"
    query: "My Import Source"
"""

RETURN = r"""
objects:
  description:
    - A list of returned Director import source objects.
...
def main():
    ...
    icinga_object = Icinga2APIObject(
        module=module, path="/importsources", data=[]
    )
