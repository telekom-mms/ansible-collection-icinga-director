...
DOCUMENTATION = """
---
module: icinga_syncrule_info
short_description: Query sync rules in Icinga2 Director
description:
   - Get a list of sync rule objects from Icinga2 Director through the director API.
...
options:
  query:
    description:
      ...
      - The text is matched on rule_name.
      - Only sync rules containing this text will be returned in the resultset.
...
EXAMPLES = """
- name: Query all sync rules in icinga
  telekom_mms.icinga_director.icinga_syncrule_info:
    url: "{{ icinga_url }}"
    url_username: "{{ icinga_user }}"
    url_password: "{{ icinga_pass }}"

- name: Query a specific sync rule in icinga
  telekom_mms.icinga_director.icinga_syncrule_info:
    url: "{{ icinga_url }}"
    url_username: "{{ icinga_user }}"
    url_password: "{{ icinga_pass }}"
    query: "Sync Hosts from CMDB"
"""

RETURN = r"""
objects:
  description:
    - A list of returned Director sync rule objects.
...
def main():
    ...
    icinga_object = Icinga2APIObject(
        module=module, path="/syncrules", data=[]
    )
