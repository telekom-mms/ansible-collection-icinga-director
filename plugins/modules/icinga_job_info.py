...
DOCUMENTATION = """
---
module: icinga_job_info
short_description: Query director jobs in Icinga2 Director
description:
   - Get a list of director job objects from Icinga2 Director through the director API.
...
options:
  query:
    description:
      ...
      - The text is matched on job_name.
      - Only jobs containing this text will be returned in the resultset.
...
EXAMPLES = """
- name: Query all director jobs in icinga
  telekom_mms.icinga_director.icinga_job_info:
    url: "{{ icinga_url }}"
    url_username: "{{ icinga_user }}"
    url_password: "{{ icinga_pass }}"

- name: Query a specific director job in icinga
  telekom_mms.icinga_director.icinga_job_info:
    url: "{{ icinga_url }}"
    url_username: "{{ icinga_user }}"
    url_password: "{{ icinga_pass }}"
    query: "Run CMDB Import"
"""

RETURN = r"""
objects:
  description:
    - A list of returned Director job objects.
