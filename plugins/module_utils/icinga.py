        ...
        if self.exists():
            ...
            if state == "absent":
                if self.module.check_mode:
                    ...
                    self.module.exit_json(changed=True, object_name=self.data['object_name'], diff=diff_result)
                else:
                    try:
                        ret = self.delete()
                        if ret['code'] == 200:
                ...
                if self.module.check_mode:
                    if diff_result:
                        changed = True
                    self.module.exit_json(changed=changed, object_name=self.data['object_name'], data=self.data, diff=diff_result)
